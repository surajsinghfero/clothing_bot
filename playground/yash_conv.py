...
def list_item_selectd(req):
    try:
        key = req.get("originalDetectIntentRequest").get("payload").get("inputs")[0].get('arguments')[0].get(
            "textValue")
        return key
...
def basicCard(speech=None, title=None, formatted_text=None, subtitle=None, card_image=None, image_scale=None,
              image_text=None, link_title=None, link_url=None, outputContexts=None, suggestions=None, response=True):
    if speech is None:
        raise ValueMissing("Speech can not be empty")
...
def listCard(title=None, speech=None, items=None, outputContexts=None):
    ...
    for i in items.iterator():
        temps.append({"optionInfo": {"key": i.id, "synonyms": i.synonyms}, "title": title,
                      "description": i.description, "image": {"url": i.image_url,
                                                              "accessibilityText": i.image_text}})
        if title is None:
...
def login_granted(req):
    status = req.get("originalDetectIntentRequest").get("payload").get("inputs")[0].get("arguments")[0].get(
